...
enum abstract TextureTypes(UInt) from UInt to UInt {
	...
	public static function isProcedural(tex:TextureTypes) {
		switch (tex) {
			case TextureTypes.Gradient | TextureTypes.PerlinNoise1 | QuasiCrystal | VoronoiDots | VoronoiPolys | WavyLines | WoodGrain | PlasmaNoise | Pool | Bio | FractalNoise | LightSquares | Machine | SweptNoise | Abstract | Dashes | LayeredNoise | SquareRegress | Swirly:
				return true;
			default:
				return false;
		}
