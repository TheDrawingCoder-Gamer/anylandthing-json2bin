...
class NanoJpeg {
	...
	function new() {
		...
		njZZ = haxe.ds.Vector.fromArrayCopy([
	...
	inline function njRowIDCT(bp) {
		...
		if (((x1 = block[bp + 4] << 11) | (x2 = block[bp + 6]) | (x3 = block[bp + 2]) | (x4 = block[bp + 1]) | (x5 = block[bp + 7]) | (x6 = block[bp + 5]) | (x7 = block[bp
			+ 3])) == 0) {
	...
	inline function njColIDCT(bp, out:FastBytes, po, stride) {
		var x0, x1, x2, x3, x4, x5, x6, x7, x8;
		if (((x1 = block[bp + 8 * 4] << 8) | (x2 = block[bp + 8 * 6]) | (x3 = block[bp + 8 * 2]) | (x4 = block[bp + 8 * 1]) | (x5 = block[bp + 8 * 7]) | (x6 = block[bp
			+ 8 * 5]) | (x7 = block[bp + 8 * 3])) == 0) {
		...
		po += stride;
		out[po] = njClip(((x3 + x2) >> 14) + 128);
		po += stride;
		out[po] = njClip(((x0 + x4) >> 14) + 128);
		po += stride;
		out[po] = njClip(((x8 + x6) >> 14) + 128);
		po += stride;
		out[po] = njClip(((x8 - x6) >> 14) + 128);
		po += stride;
		out[po] = njClip(((x0 - x4) >> 14) + 128);
	...
	function njDecodeScan() {
		...
		var count = get(1);
		var other = get(2);
		if ((!progressive && start != 0) || (count != 63 - start) || other != 0)
			notSupported();
		njSkip(length);
