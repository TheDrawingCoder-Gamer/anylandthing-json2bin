...
class FontParser {
	...
	public static function parse(bytes:haxe.io.Bytes, path:String, resolveTile:String->Tile):Font {
		...
		switch (bytes.getInt32(0)) {
			case 0x544E4642: // Internal BFNT
				return FontReader.parse(bytes, function(tp:String) {
			...
			case 0x6D783F3C, // <?xml : XML file
				...
				if (xml.hasNode.info) {
					...
					for (c in chars) {
						var t = tile.sub(Std.parseInt(c.att.x), Std.parseInt(c.att.y), Std.parseInt(c.att.width), Std.parseInt(c.att.height),
							Std.parseInt(c.att.xoffset), Std.parseInt(c.att.yoffset));
						var fc = new Font.FontChar(t, Std.parseInt(c.att.xadvance));
					...
					for (c in xml.elements) {
						...
						var t = tile.sub(Std.parseInt(r[0]), Std.parseInt(r[1]), Std.parseInt(r[2]), Std.parseInt(r[3]), Std.parseInt(o[0]),
							Std.parseInt(o[1]));
						var fc = new Font.FontChar(t, Std.parseInt(c.att.width));
				...
				for (line in lines) {
					...
					switch (line.substr(0, idx)) {
						...
						case "char":
							...
							var fc = new Font.FontChar(t, xadvance);
							glyphs.set(id, fc);
						case "kerning":
					...
					switch (id) {
						...
						case 4: // chars
							...
							while (count > 0) {
								...
								var t = tile.sub(bytes.readUInt16(), bytes.readUInt16(), bytes.readUInt16(), bytes.readUInt16(), bytes.readInt16(),
									bytes.readInt16());
								var fc = new Font.FontChar(t, bytes.readInt16());
								glyphs.set(cid, fc);
		...
		if (glyphs.get(" ".code) == null)
			glyphs.set(" ".code, new Font.FontChar(tile.sub(0, 0, 0, 0), font.size >> 1));
...
class FontReader {
	...
	public static inline function parse(bytes:haxe.io.Bytes, resolveTile:String->Tile):Font {
		return new FontReader(new haxe.io.BytesInput(bytes)).read(resolveTile);
	}
}
